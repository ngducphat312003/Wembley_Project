    ...
    for i, det in enumerate(pred):
        ...
        if len(det):
            ...
            for j, (*xyxy, conf, cls) in enumerate(reversed(det)):
                ...
                detection = (xyxy, conf, cls, j)

                if class_name == 'foamtrays':
                    foamtrays_detections.append(detection)
                else:
                ...
                #     continue
            for (xyxy, conf, cls, j) in other_detections + foamtrays_detections:
                ...
                file_name = f'{names[int(cls)]}_{os.path.basename(source_image_path).split(".")[0]}_{j}.bmp'
                cv2.imwrite(os.path.join(output_dir2, file_name), cropped_img)

                results.append((names[int(cls)], conf.item(), file_name, xyxy))    
...
def setup_ad():
    ...
    # memory_bankbl = torch.load(r"H:\HK242\LVTN\Model\blackplastic1\memory_bankblv1_1.pt").cuda()
    # memory_bankbr = torch.load(r"H:\HK242\LVTN\Model\blackplastic1\memory_bankbrv1_2.pt").cuda()
    # memory_bankmid = torch.load(r"H:\HK242\LVTN\Model\blackplastic1\memory_bankmidv1_2.pt").cuda()
    # memory_banktl = torch.load(r"H:\HK242\LVTN\Model\blackplastic1\memory_banktlv1_2.pt").cuda()
    # memory_banktr = torch.load(r"H:\HK242\LVTN\Model\blackplastic2\memory_banktlv3.pt").cuda()
...
#     plastic_config = {
#     "blplastic_": (memory_bankbl, 12.854572296),
#     "brplastic_": (memory_bankbr, 12.45312771),
#     "midplastic_": (memory_bankmid, 13.345383644104004),
#     "tlplastic_": (memory_banktl, 12.23159122467041),
#     "trplastic_": (memory_banktr, 11.510721206665039),
# }
    # plastic_config = {
    # "tlplastic_": (memory_banktr, 14.034555435180664),
    # }
