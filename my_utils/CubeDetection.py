...
def run_inference(model, device, half, stride, imgsz, source_image_path, opt):
    ...
    for i, det in enumerate(pred):
        ...
        if len(det):
            ...
            for j, (*xyxy, conf, cls) in enumerate(reversed(det)):
                if names[int(cls)] == 'midplastic' and conf.item() <= 0.85:
                    continue
                elif names[int(cls)] == 'foamtrays':
                    continue
                elif names[int(cls)] == 'brplastic' and conf.item() <= 0.75:
                    continue
                elif names[int(cls)] == 'blplastic' and conf.item() <= 0.75:
                    continue
                elif names[int(cls)] == 'tlplastic' and conf.item() <= 0.75:
                    continue
                elif names[int(cls)] == 'trplastic' and conf.item() <= 0.75:
                    continue
    ...
    #                 # Lưu ảnh với tên dựa trên class
                ...
                #     os.path.join(
                #         output_dir,
                #         f'{names[int(cls)]}_{os.path.basename(source_image_path).split(".")[0]}_{j}.jpg'
                #     ),
                #     cropped_img
                # )
                filename = f'{names[int(cls)]}_{os.path.basename(source_image_path).split(".")[0]}_{j}.bmp'
                file_path = os.path.join(output_dir, filename)
                cv2.imwrite(file_path, gray_3channel)
                results.append(((names[int(cls)], conf.item()), xyxy, filename))
